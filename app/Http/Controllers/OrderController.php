...
class OrderController extends Controller
{
	...
	public function checkout() {
		...
		if(Session::get('cart') && count(Session::get('cart'))>0) {
			...
			return view('shop-views.checkout',['cart'=>Session::get('cart'),'countries'=>$countries,'language'=>$language,'product_detail'=>$product_detail]);
	...
	public function success() {
		return view('account.success');
            ...
            $stripe_secret_key =
				...
				$stripe = new \Stripe\StripeClient("sk_test_51IpegfB7ntYBqOpJwsrkSigMygM6SAEpaTzZxgFEjciqAA6heNGt0ytGn6WirHpxDgXpiIj4ER21uNu4Li9iOTEt00FpDBZOUe");
				
                $charge_amount = ($subtotal*100);

                $charge_status = $stripe->charges->create([
				 ...
				 $stripe_charge_status = strtolower($charge_status->status);
				 ...
				 if($stripe_charge_status == "succeeded")
                     {
						...
						$order->transaction_id = $stripe_charge_status->id;
